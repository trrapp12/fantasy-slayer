...
console.log(
  'hasNotDisplayedTheMessageBefore is: ',
  hasNotDisplayedTheMessageBefore,
  'and displayNoManaMessage is: ',
  displayNoManaMessage
);
...
function castSpells() {
  ...
  console.log(
    'inside castSpells(), numberOfSpellsCast is: ',
    numberOfSpellsCast
  );
  ...
  setTimeout(() => {
    ...
    hero.spells.setCardChoiceHandler(
      hero.spells.handleCardChoice(
        hero,
        nextThreeCards,
        villain,
        render,
      ...
      hero.spells.removeAppendedCards
    );
...
function attack() {
  if (!isWaiting) {
    // creates a pause
    if (
      hero.numberOfTurns % 5 === 0 &&
      hero.numberOfTurns > 0 &&
      shuffledSpellArr.length !== 0
    ) {
      ...
      disableAttackButton();
      handleFlyOuts();
      let containerWidth = returnContainerWidth(
        document.querySelector(
          '#character-stats-health > div.character-stats--health-graph'
        )
      );
      console.log('returnContainerWidth in Render, ', containerWidth);
...
function setNextCharacter() {
  ...
  return nextCharacterData
    ? new Character(nextCharacterData, new Spells())
    : {};
}

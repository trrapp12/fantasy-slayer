...
class Character {
  constructor(data, spells = null) {
    ...
    this.diceArrayForRendering = renderDicePlaceHolderArray(
      this.totalDiceCount
    );
  ...
  getDiceHTML() {
    ...
    this.indicesToChange = this.getIndexesOfDiceScoreMatches(
    ...
    this.indicesToChange.forEach((index) => {
      if (this.diceArray[index])
        this.diceArray[index] = this.diceArray[index].replace(
          '<div class="dice">',
          '<div class="dice highlighted">'
        );
    });
  ...
  takeDamage(attackScoreArray, defendDiceScore) {
    ...
    if (hasDuplicates(attackScoreArray)) {
      ...
      this.totalDamage = calculateEnhancedScore(
        findDuplicateIndices(attackScoreArray),
        attackScoreArray
      );
      ...
      this.bufferedDamage =
        this.totalDamage - this.totalDamage * (this.defendDiceScore[0] * 0.1);
  ...
  renderCharacter() {
    ...
    if (renderBanner === true && characterClass === 'hero') {
      heroDisplayLogic = `<div class="power-hit-hero-container"><p class="power-hit-hero">The Spinner, the Giver, and the Inflexible looked warmly upon your fate and blessed your dice with matching pairs.  Your ${characterName}'s attack is increased to</p><div class="message">${
        messages ? messages : ''
      }</div></div>
            `;
    } else {
    ...
    if (renderBanner === true && characterClass === 'villain') {
      villainDisplayLogic = `<div class="power-hit-villain-container"><p class="power-hit-villain">The Furies, the Fates, the Death-Daimones and Thanatos himself have conspired for your demise.  ${characterName}'s attack is increased to</p><div class="message">${
        messages ? messages : ''
      }</div></div>`;
    } else {
      villainDisplayLogic = '';
    ...
    const graphContainer = document.querySelectorAll(
      '.character-stats--health-graph'
    );
    ...
    if (graphContainer.length < 3) {
      const containerElement = document.querySelector(
        '#main-container > div.middle-third > div.middle-third-left-column'
      );
      //   let containerWidth = returnContainerWidth(containerElement);
      graph = renderHealthChart(health, originalHealth, containerElement);
    }
    return `
            ...
            <h2>${characterName}</h2>
                <div class="character-card" style="flex-direction: ${characterCardFlexDirection}">
                    ...
                    <div class="character-stats--container" style="order: ${cssOrder}">
                        <ul>
                            <li class="attributes alive"><p class="attributes-key">Status: </p><p class="attributes-value ${
                              health > 1 * originalHealth
                                ? 'empowered'
                                : health >= 0.75 * originalHealth
                                ? 'belligerent'
                                : health >= 0.5 * originalHealth
                                ? 'unwielding'
                                : health >= 1
                                ? 'distraught'
                                : 'ofslægen-slain'
      ...
      health > 1 * originalHealth
        ...
        : 'ofslægen / slain'
                            ...
                            <li class="attributes skill"><p class="attributes-key">Skills: </p><p class="attributes-value"> ${skill.join(
                              ' '
                            )}</p></li>
                            ...
                            <li class="attributes weakness"><p class="attributes-key">Weakness: </p><p class="attributes-value"> ${weakness.join(
                              ' '
                            )}</p></li>
                    ...
                    <div class="both-dice-container">
                        ...
                        <div class="dice-container">
                            ...
                            <p class="attack-defend-label"> <strong> Werede  ( Defend ): </strong> ${
                              characterName === 'Zedfire, Hælend of darkness'
                                ? 'The Dark Lord'
                                : 'You'
                            } defended ${defendDiceValue}0%</p>
